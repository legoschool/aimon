...
const BALANCE = {
  ...
  /* 잡으려면 적어도 이만큼은 풀어야 한다.
     운으로 두 문제 맞혀 바로 던지는 길을 막으려는 장치다.

     처음엔 4로 뒀는데, 상성이 맞으면 두 문제로 장악력이 0이 되어
     "이미 쓰러진 몬스터 앞에서 두 문제를 더 풀라"는 말이 나왔다.
     열심히 푼 아이에게 벌처럼 느껴져서 3으로 낮췄다.

     찍는 것을 막는 일은 읽기 잠금·튕겨 누르기 감지·문항 설계가
     이미 하고 있으므로 이 숫자가 혼자 버틸 필요는 없다. */
  minAskedToCatch: 3,
...
/* 가치볼 3종 */
const BALLS = {
  basic: { id: "basic", name: "가치볼", rate: 1.0, start: 6 },
